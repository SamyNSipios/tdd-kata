...
class StringCalculatorTest {
    ...
    void addWithNullValue() {
        try {
            assertEquals(0, StringCalculator.add(""), "Should return 0 if no value is input");
        }
    ...
    void addWithOneValue() {
        try {
            assertEquals(1, StringCalculator.add("1"), "Should return 1 if 1 is input");
            assertEquals(5, StringCalculator.add("5"), "Should return 5 if 5 is input");
            assertEquals(0, StringCalculator.add("a"), "Should return 0 if input is incorrect");
        }
        catch (NegativeValueException exception) {
            assertTrue(exception.getMessage().isEmpty());
        }
    ...
    void addWithTwoOrMoreValue() {
        try {
            assertEquals(6, StringCalculator.add("1     ,2     ,3"),"Should return 6 if 1,2,3 is input");
            assertEquals(10, StringCalculator.add("5,5"),"Should return 10 if 5,5 is input");
            assertEquals(0, StringCalculator.add("a,6"),"Should return 6 if input is 6 and incorrect");
        }
        catch (NegativeValueException exception) {
            assertTrue(exception.getMessage().isEmpty());
        }
    ...
    void addWithCornerCase() {
        try {
            ...
            assertEquals(10, StringCalculator.add("5,5"),"Should return 10 if 5,5 is input");
            assertEquals(0, StringCalculator.add("a,6"),"Should return 6 if input is 6 and incorrect");
            //assertEquals(0, StringCalculator.add("2, 2"), "Should return -2");
        }
        catch (NegativeValueException exception) {
            assertTrue(exception.getMessage().isEmpty());
        }
    ...
    void addShouldThrowOnNegativeValues() {
        try {
            Exception exception = assertThrows(Exception.class, () -> StringCalculator.add("-2, -1"));
    ...
    void addShouldIgnoreValueSuperiorAt1000 () {
        try {
            assertEquals(2, StringCalculator.add("2, 1001"), "Should ignore the 1001");
        }
        catch (NegativeValueException exception) {
